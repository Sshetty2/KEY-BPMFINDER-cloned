...
// .2 This function will first utilizes chrome's messaging api to 'send a message' called 'launchOauth'(what happens to the message is unclear at this point, but, presumably, it will handled by the content script). 
...
// This script stores an event listener in memory that utilizes the chrome alarm api to 'listen' for an alarm. Once the alarm is fired, a message is logged to the console and chrome's storage api is queried once again. 'status' in chrome's local storage object is set to 0 and the 'disabled' attribute of the 'button' is removed so that it can be clickable once again and another message indicating the status of the alarm and the current state of local storage is logged to the console.
...
// this variable declaration sets the the button's 'onclick' event equal to the function 'handler' defined per above 
